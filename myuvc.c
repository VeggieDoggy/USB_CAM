...
static int myuvc_probe(struct usb_interface *intf, const struct usb_device_id *id) {
    ...
    for (i = 0; i < descriptor->bNumConfigurations; i++)
    {
        ...
        if ((buffer[1] == USB_DT_CS_INTERFACE) && (interface->bInterfaceSubClass == 1)) { // VideoControl Interface
            parse_videocontrol_interface(intf, buffer, buflen);
        }
        if ((buffer[1] == USB_DT_CS_INTERFACE) && (interface->bInterfaceSubClass == 2)) { // VideoStreaming Interface
